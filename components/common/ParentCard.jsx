...
export const ParentCard = ({ data, isMale = false }) => {
  return (
    <div className="parent-card">
      <div
        className="parent-card__img"
        style={{
          backgroundImage: `url(https://res.cloudinary.com/leninsdo/image/upload/${config.imgID}/petstory/${data.mainImageGuid})`,
        }}
      ></div>
      <div className="parent-card__main">
        <div className="parent-card__name text text_type_name">
          {data.name} {isMale ? "(папа)" : "(мама)"}
        </div>
        {data.city && (
        ...
        {data.birthDate && (
          <div className="parent-card__birth text_type_desc">
        ...
        {data.pedigreeUrl && (
          <Link href={data.pedigreeUrl}>
            <a
              className="parent-card__pedigree text text_type_desc text_color_main"
              target="_blank"
            >
              Родословная
            </a>
        ...
        {data.description && (
          <div className="text_type_h6 text text_color_gray parent-card__regalia">
            {data.description}
          </div>
        )}
